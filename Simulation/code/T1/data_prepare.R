setwd("/public/home/liujf/workspace/xueyh/TempWork/h_mat_sim_c2021")
...
tbv <- read_delim("data/tbv.txt", show_col_types = F, col_names = F) %>% 
  mutate(id = row_number())

pheno_all <- read_delim("data/my_bv.txt", show_col_types = F) %>% 
...
pedi <- read_delim("data/ped", col_names = F, show_col_types = F) %>% 
...
if(sce == 1) {
    ...
    gmat <- fread("data/G", data.table = F) %>%
    ...
    tmat <- fread("data/M.txt", data.table = F) %>%
      mutate(id = row_number()) %>% 
