...
print(sprintf(' 当前MegaLMM   Trial 为 Trial %d, Fold(SLURM_ARRAY_TASK_ID) %d, K %d',trial,foldid,k))
print(sprintf(' using threat num: %d',(RcppParallel::defaultNumThreads()-1)))
# MD2014
#trial = 6
...
#BLUEs$partition = sample(c('Validation','Training'),size=nrow(BLUEs),replace=T)
...
runID = sprintf('MegaLMM_Krause_X_%d',foldid)
...
MegaLMM_state = setup_model_MegaLMM(Y,            # n x p data matrix
                                    ...
                                    Lambda_fixed = matrix(c(1,rep(0,ncol(Y)-1)),nrow=1),
...
# MegaLMM_state$Posterior$posteriorSample_params = c('Lambda','F_h2','B2_F')#,'Eta_mean')
# MegaLMM_state$Posterior$posteriorSample_params = c('Eta')#,'Eta_mean')
MegaLMM_state$Posterior$posteriorMean_params = c()
MegaLMM_state$Posterior$posteriorSample_params = c('Lambda','B2_F')
MegaLMM_state$Posterior$posteriorFunctions = list(pred = 'X2_F %*% B2_F %*% Lambda[,1]')
...
results = data.frame(Method = 'MegaLMM_X',
                     pearson_h2 = cor(BLUEs$BLUP[nas],U[nas,1])/sqrt(h2_BLUE),
                     pearson = cor(BLUEs$BLUP[nas],U[nas,1]),
...
results$fold = foldid
write.csv(results,file = sprintf('%s/results_MegaLMM_X_fold_%d.csv',results_dir,foldid))
